...
return false;
		...
		$query = "
			...
			FROM ".Game::GAME_TABLE." AS G
				LEFT JOIN ".Game::GAME_PLAYER_TABLE." AS GP
					USING (game_id)
			WHERE GP.player_id = '{$this->id}'
				AND G.state NOT IN ('Finished')
		";
	...
	static public function get_maxed( )
	{
		...
		if (true || $invites_count_toward_max_games) {
			$query = "
				SELECT COUNT(GP.game_id) AS invite_count
					, PE.player_id
...
--
-- Table structure for table `po_po_player`
--
